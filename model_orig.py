# from https://gist.github.com/xmfbit/67c407e34cbaf56e7820f09e774e56d8
...
class Bottleneck(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.activ(self.bn1(x))
        out = self.conv1(out)

        out = self.activ(self.bn2(out))
        out = self.conv2(out)

        out = self.activ(self.bn3(out))
        out = self.conv3(out)
...
class ResNet(nn.Module):
    ...
    def _make_layer(self, block, inplanes, outplanes, nstage, stride=1):
        layers = []
        layers.append(block(inplanes, outplanes, self.activ, stride))
        for i in range(1, nstage):
            layers.append(block(outplanes, outplanes, self.activ, stride=1))
